...
const float x_default = /*62*/ 62, x_offset = 0;
...
//BLUETOOTH
void serialFlush(){                     //delete extra input
...
void loop()
{
  ...
  if(Serial.available() > 0){
    ...
    else if(str == "SIT DOWN"){
      sit();
      serialFlush();
    }
    else if (str == "MOVE FORWARD"){
      step_forward(6);
      serialFlush();
    }
    else if(str == "WAVE"){
...
void stand(void)
{
  ...
  for (int leg = 0; leg < 4; leg++)
  {
    set_site(leg, KEEP, KEEP, z_default - 20);
  }
  wait_all_reach();
}
